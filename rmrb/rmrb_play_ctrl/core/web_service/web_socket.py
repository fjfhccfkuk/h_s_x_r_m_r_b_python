...
class WebSocket(object):
   ...
   def _handleData(self):
      ...
      if self.handshaked is False:
         ...
         print "need to recv data."
         if not data:
            print "_handleData data is empty"
            raise Exception('remote socket closed')

         else:
            # accumulate
            print "_handleData data:%s" % str(data)
          ...
          try:
            ...
            if VER >= 3:
             ...
             for d in data:
              ...
              print "_handleData handshaked excp:%s" % e.message
   ...
   def _parseMessage(self, byte):
      ...
      # PAYLOAD STATE
         ...
         print " _parseMessage state PAYLOAD, byte:0x%x" % byte + " c:%c" % byte + " index:%d" % self.index + " self.data len:%d" % len (self.data)
         if self.hasmask is True:
...
from interface import InterfaceWebSer
from threading import Lock
class SimpleWebSocketServer(InterfaceWebSer):

   def __init__(self, host, port, websocketclass, selectInterval = 0.5):
      ...
      self.TOGGLE_COUNT=10
      self.loop_times=0
      self.__media_sync_lock = Lock()
      self.__fresh_media = None

   """ override web_service.interface"""
   def send_media(self, abspath):
      self.__media_sync_lock.acquire(True)
      self.__fresh_media = abspath
      self.__media_sync_lock.release()
   ...
   def serveonce(self):
      ...
      for fileno in self.listeners:
         ...
         self.loop_times += 1;
#         print "listened a connection ,fid:%d" % fileno
         client = self.connections[fileno]
         if client.sendq:
            writers.append(fileno)
         if client.handshaked:
             self.__media_sync_lock.acquire(True)
             if self.__fresh_media:
                 client.sendMessage(unicode(self.__fresh_media))
                 self.__fresh_media = None
                 writers.append(fileno)
             self.__media_sync_lock.release()
...
class SimpleEcho(WebSocket):
   def handleMessage(self):
      try:
         ...
         print "handleMessage excp:%s" % e.message
...
def run_web_service():
    ...
    cls = SimpleEcho
    webServer = SimpleWebSocketServer('127.0.0.1', 21000, cls)
    ...
    webThread = Thread(target=webServer.serveforever)
    webThread.start()
    return webServer
