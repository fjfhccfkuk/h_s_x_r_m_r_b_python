...
import rmrb_play_ctrl.core.dns.ttl
...
class UngetBufferFull(rmrb_play_ctrl.core.dns.exception.DNSException):
...
class Token(object):
    ...
    def unescape(self):
        ...
        while i < l:
            ...
            if c == '\\':
                if i >= l:
                    raise rmrb_play_ctrl.core.dns.exception.UnexpectedEnd
                c = self.value[i]
                i += 1
                if c.isdigit():
                    if i >= l:
...
class Tokenizer(object):
    ...
    def get(self, want_leading=False, want_comment=False):
        ...
        @rtype: Token object
        @raises rmrb_play_ctrl.core.dns.exception.UnexpectedEnd: input ended prematurely
        @raises rmrb_play_ctrl.core.dns.exception.SyntaxError: input was badly formed
        """
        ...
        while True:
            ...
            if c == '' or c in self.delimiters:
                ...
                if c == '\\':
                    ...
                    raise rmrb_play_ctrl.core.dns.exception.SyntaxError('newline in quoted string')
            elif c == '\\':
                #
        ...
        if token == '' and ttype != QUOTED_STRING:
            if self.multiline:
                raise rmrb_play_ctrl.core.dns.exception.SyntaxError('unbalanced parentheses')
            ttype = EOF
    ...
    def get_int(self):
        ...
        if not token.is_identifier():
            raise rmrb_play_ctrl.core.dns.exception.SyntaxError('expecting an identifier')
        if not token.value.isdigit():
            raise rmrb_play_ctrl.core.dns.exception.SyntaxError('expecting an integer')
    ...
    def get_string(self, origin=None):
        ...
        if not (token.is_identifier() or token.is_quoted_string()):
            raise rmrb_play_ctrl.core.dns.exception.SyntaxError('expecting a string')
        return token.value
    ...
    def get_name(self, origin=None):
        ...
        if not token.is_identifier():
            raise rmrb_play_ctrl.core.dns.exception.SyntaxError('expecting an identifier')
        return rmrb_play_ctrl.core.dns.name.from_text(token.value, origin)
    ...
    def get_ttl(self):
        ...
        if not token.is_identifier():
            raise rmrb_play_ctrl.core.dns.exception.SyntaxError('expecting an identifier')
        return rmrb_play_ctrl.core.dns.ttl.from_text(token.value)
