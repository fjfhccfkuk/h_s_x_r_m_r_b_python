...
@type default_resolver: rmrb_play_ctrl.core.dns.resolver.Resolver object"""
...
import rmrb_play_ctrl.core.dns.ipv4
import rmrb_play_ctrl.core.dns.ipv6
import rmrb_play_ctrl.core.dns.message
import rmrb_play_ctrl.core.dns.name
import rmrb_play_ctrl.core.dns.query
import rmrb_play_ctrl.core.dns.rcode
import rmrb_play_ctrl.core.dns.rdataclass
import rmrb_play_ctrl.core.dns.rdatatype
import rmrb_play_ctrl.core.dns.reversename
import rmrb_play_ctrl.core.dns.tsig
from rmrb_play_ctrl.core.dns._compat import xrange, string_types
...
class NXDOMAIN(rmrb_play_ctrl.core.dns.exception.DNSException):
    ...
    def canonical_name(self):
        ...
        IN = rmrb_play_ctrl.core.dns.rdataclass.IN
        CNAME = rmrb_play_ctrl.core.dns.rdatatype.CNAME
        cname = None
        for qname in self.kwargs['qnames']:
            ...
            if cname is not None:
                return rmrb_play_ctrl.core.dns.name.from_text(cname)
...
class YXDOMAIN(rmrb_play_ctrl.core.dns.exception.DNSException):

    """The DNS query name is too long after DNAME substitution."""

# The definition of the Timeout exception has moved from here to the
# rmrb_play_ctrl.core.dns.exception module.  We keep rmrb_play_ctrl.core.dns.resolver.Timeout defined for
# backwards compatibility.

Timeout = rmrb_play_ctrl.core.dns.exception.Timeout


class NoAnswer(rmrb_play_ctrl.core.dns.exception.DNSException):
...
class NoNameservers(rmrb_play_ctrl.core.dns.exception.DNSException):
...
class NotAbsolute(rmrb_play_ctrl.core.dns.exception.DNSException):

    """An absolute domain name is required but a relative name was provided."""


class NoRootSOA(rmrb_play_ctrl.core.dns.exception.DNSException):

    """There is no SOA RR at the DNS root name. This should never happen!"""


class NoMetaqueries(rmrb_play_ctrl.core.dns.exception.DNSException):
...
class Answer(object):
    ...
    @type qname: rmrb_play_ctrl.core.dns.name.Name object
    @ivar rdtype: The query type
    @type rdtype: int
    @ivar rdclass: The query class
    @type rdclass: int
    @ivar response: The response message
    @type response: rmrb_play_ctrl.core.dns.message.Message object
    ...
    @type canonical_name: rmrb_play_ctrl.core.dns.name.Name object
    """
    ...
    def __init__(self, qname, rdtype, rdclass, response,
        ...
        for count in xrange(0, 15):
            try:
                ...
                if rdtype != rmrb_play_ctrl.core.dns.rdatatype.CNAME:
                    try:
                        crrset = response.find_rrset(response.answer,
                                                     qname,
                                                     rdclass,
                                                     rmrb_play_ctrl.core.dns.rdatatype.CNAME)
            ...
            while 1:
                ...
                try:
                    srrset = response.find_rrset(response.authority, qname,
                                                 rdclass, rmrb_play_ctrl.core.dns.rdatatype.SOA)
                    ...
                    try:
                        qname = qname.parent()
                    except rmrb_play_ctrl.core.dns.name.NoParent:
                        break
...
class Cache(object):
    ...
    def get(self, key):
        ...
        @param key: the key
        @type key: (rmrb_play_ctrl.core.dns.name.Name, int, int) tuple whose values are the
        query name, rdtype, and rdclass.
        @rtype: rmrb_play_ctrl.core.dns.resolver.Answer object or None
        """
    ...
    def put(self, key, value):
        ...
        @type value: rmrb_play_ctrl.core.dns.resolver.Answer object
        """
    ...
    def flush(self, key=None):
        ...
        @type key: (rmrb_play_ctrl.core.dns.name.Name, int, int) tuple or None
        """
...
class Resolver(object):
    ...
    @type domain: rmrb_play_ctrl.core.dns.name.Name object
    ...
    @type search: list of rmrb_play_ctrl.core.dns.name.Name objects
    ...
    @type cache: rmrb_play_ctrl.core.dns.resolver.Cache object
    ...
    def reset(self):
        ...
        self.domain = \
            rmrb_play_ctrl.core.dns.name.Name(rmrb_play_ctrl.core.dns.name.from_text(socket.gethostname())[1:])
        if len(self.domain) == 0:
            self.domain = rmrb_play_ctrl.core.dns.name.root
        self.nameservers = []
        ...
        try:
            for l in f:
                ...
                if tokens[0] == 'nameserver':
                    ...
                    self.domain = rmrb_play_ctrl.core.dns.name.from_text(tokens[1])
                elif tokens[0] == 'search':
                    for suffix in tokens[1:]:
                        self.search.append(rmrb_play_ctrl.core.dns.name.from_text(suffix))
    ...
    def _config_win32_domain(self, domain):
        ...
        self.domain = rmrb_play_ctrl.core.dns.name.from_text(str(domain))
    ...
    def _config_win32_search(self, search):
        ...
        for s in search_list:
            if s not in self.search:
                self.search.append(rmrb_play_ctrl.core.dns.name.from_text(s))
    ...
    def query(self, qname, rdtype=rmrb_play_ctrl.core.dns.rdatatype.A, rdclass=rmrb_play_ctrl.core.dns.rdataclass.IN,
        ...
        @rtype: rmrb_play_ctrl.core.dns.resolver.Answer instance
        ...
        if isinstance(qname, string_types):
            qname = rmrb_play_ctrl.core.dns.name.from_text(qname, None)
        if isinstance(rdtype, string_types):
            rdtype = rmrb_play_ctrl.core.dns.rdatatype.from_text(rdtype)
        if rmrb_play_ctrl.core.dns.rdatatype.is_metatype(rdtype):
            raise NoMetaqueries
        if isinstance(rdclass, string_types):
            rdclass = rmrb_play_ctrl.core.dns.rdataclass.from_text(rdclass)
        if rmrb_play_ctrl.core.dns.rdataclass.is_metaclass(rdclass):
            raise NoMetaqueries
        qnames_to_try = []
        if qname.is_absolute():
            ...
            if len(qname) > 1:
                qnames_to_try.append(qname.concatenate(rmrb_play_ctrl.core.dns.name.root))
            if self.search:
        ...
        for _qname in qnames_to_try:
            if self.cache:
                ...
                if answer is not None:
                    if answer.rrset is None and raise_on_no_answer:
                        ...
                        return answer
            request = rmrb_play_ctrl.core.dns.message.make_query(_qname, rdtype, rdclass)
            ...
            while response is None:
                ...
                for nameserver in nameservers[:]:
                    ...
                    try:
                        tcp_attempt = tcp
                        if tcp:
                            response = rmrb_play_ctrl.core.dns.query.tcp(request, nameserver,
                                                     timeout, port,
                                                     source=source,
                                                     source_port=source_port)
                        else:
                            response = rmrb_play_ctrl.core.dns.query.udp(request, nameserver,
                                                     timeout, port,
                                                     source=source,
                                                     source_port=source_port)
                            if response.flags & rmrb_play_ctrl.core.dns.flags.TC:
                                # Response truncated; retry with TCP.
                                tcp_attempt = True
                                timeout = self._compute_timeout(start)
                                response = \
                                    rmrb_play_ctrl.core.dns.query.tcp(request, nameserver,
                                                  timeout, port,
                                                  source=source,
                                                  source_port=source_port)
                    except (socket.error, rmrb_play_ctrl.core.dns.exception.Timeout) as ex:
                        #
                        ...
                        response = None
                        continue
                    except rmrb_play_ctrl.core.dns.query.UnexpectedSource as ex:
                        #
                        ...
                        response = None
                        continue
                    except rmrb_play_ctrl.core.dns.exception.FormError as ex:
                        #
                    ...
                    if rcode == rmrb_play_ctrl.core.dns.rcode.YXDOMAIN:
                        ex = YXDOMAIN()
                        errors.append((nameserver, tcp_attempt, port, ex,
                                       response))
                        raise ex
                    if rcode == rmrb_play_ctrl.core.dns.rcode.NOERROR or \
                            rcode == rmrb_play_ctrl.core.dns.rcode.NXDOMAIN:
                        break
                    #
                    ...
                    #
                    if rcode != rmrb_play_ctrl.core.dns.rcode.SERVFAIL or not self.retry_servfail:
                        nameservers.remove(nameserver)
                    errors.append((nameserver, tcp_attempt, port,
                                   rmrb_play_ctrl.core.dns.rcode.to_text(rcode), response))
                    response = None
            ...
            if response.rcode() == rmrb_play_ctrl.core.dns.rcode.NXDOMAIN:
    ...
    def use_tsig(self, keyring, keyname=None,
        ...
        is rmrb_play_ctrl.core.dns.tsig.default_algorithm.
    ...
    def use_edns(self, edns, ednsflags, payload):
        """Configure Ermrb_play_ctrl.core.dns.

        @param edns: The EDNS level to use.  The default is -1, no Ermrb_play_ctrl.core.dns.
        @type edns: int
...
def query(qname, rdtype=rmrb_play_ctrl.core.dns.rdatatype.A, rdclass=rmrb_play_ctrl.core.dns.rdataclass.IN,
    ...
    @see: L{rmrb_play_ctrl.core.dns.resolver.Resolver.query} for more information on the
    parameters."""
    return get_default_resolver().query(qname, rdtype, rdclass, tcp, source,
                                        raise_on_no_answer, source_port)


def zone_for_name(name, rdclass=rmrb_play_ctrl.core.dns.rdataclass.IN, tcp=False, resolver=None):
    """Find the name of the zone which contains the specified name.

    @param name: the query name
    @type name: absolute rmrb_play_ctrl.core.dns.name.Name object or string
    @param rdclass: The query class
    @type rdclass: int
    @param tcp: use TCP to make the query (default is False).
    @type tcp: bool
    @param resolver: the resolver to use
    @type resolver: rmrb_play_ctrl.core.dns.resolver.Resolver object or None
    @rtype: rmrb_play_ctrl.core.dns.name.Name"""

    if isinstance(name, string_types):
        name = rmrb_play_ctrl.core.dns.name.from_text(name, rmrb_play_ctrl.core.dns.name.root)
    if resolver is None:
    ...
    while 1:
        try:
            answer = resolver.query(name, rmrb_play_ctrl.core.dns.rdatatype.SOA, rdclass, tcp)
            if answer.rrset.name == name:
                return name
            # otherwise we were CNAMEd or DNAMEd and need to look higher
        except (rmrb_play_ctrl.core.dns.resolver.NXDOMAIN, rmrb_play_ctrl.core.dns.resolver.NoAnswer):
            pass
        try:
            name = name.parent()
        except rmrb_play_ctrl.core.dns.name.NoParent:
    ...
    try:
        ...
        if host is None:
            ...
            if len(parts) == 2:
                ...
                ahost = host
            addr = rmrb_play_ctrl.core.dns.ipv6.inet_aton(ahost)
            v6addrs.append(host)
            canonical_name = host
    except Exception:
        try:
            # Is it a V4 address literal?
            addr = rmrb_play_ctrl.core.dns.ipv4.inet_aton(host)
            v4addrs.append(host)
            canonical_name = host
        except Exception:
            if flags & socket.AI_NUMERICHOST == 0:
                try:
                    if family == socket.AF_INET6 or family == socket.AF_UNSPEC:
                        v6 = _resolver.query(host, rmrb_play_ctrl.core.dns.rdatatype.AAAA,
                    ...
                    if family == socket.AF_INET or family == socket.AF_UNSPEC:
                        v4 = _resolver.query(host, rmrb_play_ctrl.core.dns.rdatatype.A,
                                             raise_on_no_answer=False)
                        host = v4.qname
                ...
                except rmrb_play_ctrl.core.dns.resolver.NXDOMAIN:
                    raise socket.gaierror(socket.EAI_NONAME)
                except:
...
def _getnameinfo(sockaddr, flags=0):
    ...
    if flags & socket.NI_DGRAM:
        ...
        pname = 'tcp'
    qname = rmrb_play_ctrl.core.dns.reversename.from_address(addr)
...
def _gethostbyaddr(ip):
    try:
        rmrb_play_ctrl.core.dns.ipv6.inet_aton(ip)
        sockaddr = (ip, 80, 0, 0)
        family = socket.AF_INET6
    except Exception:
        sockaddr = (ip, 80)
