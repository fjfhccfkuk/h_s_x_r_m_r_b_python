...
import rmrb_play_ctrl.core.dns.hash
...
class BadTime(rmrb_play_ctrl.core.dns.exception.DNSException):

    """The current time is not within the TSIG's validity time."""


class BadSignature(rmrb_play_ctrl.core.dns.exception.DNSException):

    """The TSIG signature fails to verify."""


class PeerError(rmrb_play_ctrl.core.dns.exception.DNSException):
...
HMAC_MD5 = rmrb_play_ctrl.core.dns.name.from_text("HMAC-MD5.SIG-ALG.REG.INT")
HMAC_SHA1 = rmrb_play_ctrl.core.dns.name.from_text("hmac-sha1")
HMAC_SHA224 = rmrb_play_ctrl.core.dns.name.from_text("hmac-sha224")
HMAC_SHA256 = rmrb_play_ctrl.core.dns.name.from_text("hmac-sha256")
HMAC_SHA384 = rmrb_play_ctrl.core.dns.name.from_text("hmac-sha384")
HMAC_SHA512 = rmrb_play_ctrl.core.dns.name.from_text("hmac-sha512")
...
def sign(wire, keyname, secret, time, fudge, original_id, error,
    ...
    if first:
        ...
        ctx.update(struct.pack('!H', rmrb_play_ctrl.core.dns.rdataclass.ANY))
...
def validate(wire, keyname, secret, now, request_mac, tsig_start, tsig_rdata,
    ...
    current = tsig_rdata
    (aname, used) = rmrb_play_ctrl.core.dns.name.from_wire(wire, current)
...
def get_algorithm(algorithm):
    ...
    if isinstance(algorithm, string_types):
        algorithm = rmrb_play_ctrl.core.dns.name.from_text(algorithm)

    try:
        return (algorithm.to_digestable(), rmrb_play_ctrl.core.dns.hash.hashes[_hashes[algorithm]])
    except KeyError:
