...
@type root: rmrb_play_ctrl.core.dns.name.Name object
@var empty: The empty DNS name.
@type empty: rmrb_play_ctrl.core.dns.name.Name object
"""
...
class EmptyLabel(rmrb_play_ctrl.core.dns.exception.SyntaxError):

    """A DNS label is empty."""


class BadEscape(rmrb_play_ctrl.core.dns.exception.SyntaxError):

    """An escaped code in a text format of DNS name is invalid."""


class BadPointer(rmrb_play_ctrl.core.dns.exception.FormError):

    """A DNS compression pointer points forward instead of backward."""


class BadLabelType(rmrb_play_ctrl.core.dns.exception.FormError):

    """The label type in DNS name wire format is unknown."""


class NeedAbsoluteNameOrOrigin(rmrb_play_ctrl.core.dns.exception.DNSException):

    """An attempt was made to convert a non-absolute name to
    wire when there was also a non-absolute (or missing) origin."""


class NameTooLong(rmrb_play_ctrl.core.dns.exception.FormError):

    """A DNS name is > 255 octets long."""


class LabelTooLong(rmrb_play_ctrl.core.dns.exception.SyntaxError):

    """A DNS label is > 63 octets long."""


class AbsoluteConcatenation(rmrb_play_ctrl.core.dns.exception.DNSException):

    """An attempt was made to append anything other than the
    empty name to an absolute DNS name."""


class NoParent(rmrb_play_ctrl.core.dns.exception.DNSException):

    """An attempt was made to get the parent of the root name
    or the empty name."""

class NoIDNA2008(rmrb_play_ctrl.core.dns.exception.DNSException):

    """IDNA 2008 processing was requested but the idna module is not
    available."""


class IDNAException(rmrb_play_ctrl.core.dns.exception.DNSException):
...
class Name(object):
    ...
    The rmrb_play_ctrl.core.dns.name.Name class represents a DNS name as a tuple of labels.
    ...
    def fullcompare(self, other):
        ...
        and is one of: rmrb_play_ctrl.core.dns.name.NAMERELN_NONE,
        rmrb_play_ctrl.core.dns.name.NAMERELN_SUPERDOMAIN, rmrb_play_ctrl.core.dns.name.NAMERELN_SUBDOMAIN,
        rmrb_play_ctrl.core.dns.name.NAMERELN_EQUAL, or rmrb_play_ctrl.core.dns.name.NAMERELN_COMMONANCESTOR
    ...
    def canonicalize(self):
        ...
        @rtype: rmrb_play_ctrl.core.dns.name.Name object
        """
    ...
    def to_unicode(self, omit_final_dot=False, idna_codec=None):
        ...
        IDNA2008 as well.
        @type idna_codec: rmrb_play_ctrl.core.dns.name.IDNA
        @rtype: string
        """
    ...
    def split(self, depth):
        ...
        l = len(self.labels)
        if depth == 0:
            return (self, rmrb_play_ctrl.core.dns.name.empty)
        elif depth == l:
            return (rmrb_play_ctrl.core.dns.name.empty, self)
...
def from_unicode(text, origin=root, idna_codec=None):
    ...
    @type origin: rmrb_play_ctrl.core.dns.name.Name
...
def from_wire(message, current):
    ...
    of the message
    @type current: int
    @raises rmrb_play_ctrl.core.dns.name.BadPointer: a compression pointer did not point backwards
    in the message
    @raises rmrb_play_ctrl.core.dns.name.BadLabelType: an invalid label type was encountered.
    @returns: a tuple consisting of the name that was read and the number
    of bytes of the wire format message which were consumed reading it
    @rtype: (rmrb_play_ctrl.core.dns.name.Name object, int) tuple
    """

    if not isinstance(message, binary_type):
        raise ValueError("input to from_wire() must be a byte string")
    message = rmrb_play_ctrl.core.dns.wiredata.maybe_wrap(message)
    labels = []
